 ...
 * { name, qty, lineTotal, desc?, note? }[]. Callers compute lineTotal themselves
 * (rather than this module deriving it from a unit price) so each caller uses
 * whichever value is exact for its source: quoteEmail.js has an exact unit price
 * (price * qty), while the Stripe receipt path has an exact line total
 * (amount_total) but no exact unit price when a line's amount doesn't divide
 * evenly by its quantity. `desc`, if present, renders as a line under the item
 * name. `note`, if present, replaces the formatted currency amount — used for
 * quote-only ($0) items so they read as "price provided after review" rather
 * than the misleading "$0.00".
 */
